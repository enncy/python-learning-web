<template>
  <CommonLayout :no-shadow="true">
    <Loading :loading="loading">
      <div v-if="userModel && userInfo">
        <div class="pt-5 shadow-sm bg-white">
        ...
        >
          ...
          <div class="col-lg-5 col-12" v-if="selectedKeys[0] === 'follow'">
            <div v-if="userModel.followingModelPage.records.length">
              <template
                v-for="following of userModel.followingModelPage.records"
              >
                <UserCard
                  class="follow-card"
                  size="small"
          ...
          <div class="col-lg-5 col-12" v-if="selectedKeys[0] === 'favorite'">
            <div v-if="userModel.favoriteModelPage.records.length">
              <PostTable
                :posts="
                  userModel.favoriteModelPage.records.map((r) => r.postModel)
                "
              >
              </PostTable>
            </div>
          ...
          <div class="col-lg-5 col-12" v-if="selectedKeys[0] === 'subscribe'">
            <div v-if="userModel.subscribeModelPage.records.length">
              <CategoryList
                :category-models="
                  userModel.subscribeModelPage.records.map(
                    (r) => r.categoryModel
                  )
                "
              ></CategoryList>
      ...
      <div v-else>
        <NotFound title="此用户不存在"></NotFound>
      </div>
    </Loading>
  </CommonLayout>
</template>
